...
C620 motor1(&can2_bus,1,-1,kWheelRadius_mm);
C620 motor2(&can2_bus,2,-1,kWheelRadius_mm);
C620 motor3(&can2_bus,3,1,kWheelRadius_mm);
C620 motor4(&can2_bus,4,1,kWheelRadius_mm);

C610 loading_motor(&can1_bus,1,1,1);
C620 pitch_motor(&can1_bus,2,1,1);


PID m1_pid(1.0, 0, 0, kControlPeriod_ms);
PID m2_pid(1.0, 0.0, 0, kControlPeriod_ms);
PID m3_pid(1.0, 0.01, 0, kControlPeriod_ms);
PID m4_pid(1.0, 0.01, 0, kControlPeriod_ms);
